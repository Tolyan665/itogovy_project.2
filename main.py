...
# Главное окно
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, text='Добавить сотрудника ', command=self.open_dialog, bg='#d7d7d7',
                                    bd=0, compound=tk.TOP, image=self.add_img)
        btn_open_dialog.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='изменение.png')
        btn_edit_dialog = tk.Button(toolbar, text='Редактировать', bg='#d7d7d7', bd=0, image=self.update_img,
                                    compound=tk.TOP, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='удаление.png')
        btn_delete_dialog = tk.Button(toolbar, text='Удалить', bg='#d7d7d7', bd=0, image=self.delete_img,
                                      compound=tk.TOP, command=self.delete_records)
        btn_delete_dialog.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='поиск.png')
        btn_search = tk.Button(toolbar, text='Поиск', bg='#d7d7d7', bd=0, image=self.search_img,
                               compound=tk.TOP, command=self.open_search_dialog)

        btn_search.pack(side=tk.LEFT)


        self.refresh_img = tk.PhotoImage(file='перезагрузка.png')
        btn_refresh = tk.Button(toolbar, text='Обновить', bg='#d7d7d7', bd=0, image=self.refresh_img,
...
# Обновление данных
    def update_record(self, employee, position, salary):
        self.db.c.execute('''UPDATE staff SET employee=?, position=?, salary=? WHERE ID=?''',
                          (employee, position, salary, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
# Удаление данных
    def delete_records(self):
        for selection_item in self.tree.selection():
            self.db.c.execute('''DELETE FROM staff WHERE id=? ''', (self.tree.set(selection_item, '#1'),))
            self.db.conn.commit()
            self.view_records()
...
# Поиск данных
    def search_records(self, employee):
        employee = ('%' + employee + '%',)
        self.db.c.execute('''SELECT * FROM staff WHERE employee LIKE ?''', employee)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.btn_ok.bind('<Button-1>', lambda event: self.view.records(self.entry_employee.get(),
                                                                       self.combobox.get(),
...
class DB:
    ...
    def insert_data(self, employee, position, salary):
        self.c.execute('''INSERT INTO staff (employee, position, salary) VALUES (?, ?, ?) ''',
